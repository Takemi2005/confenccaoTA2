...
class GraficosDashboard extends ChartWidget
{
    ...
    {
        // Últimas movimentações para montar entradas/saídas
        ...
        // Labels usando o created_at das duas séries (assumimos mesma janela temporal).
        $labels = $entradas->pluck('created_at')->map(fn ($d) => $d->format('d/m'))->all();

        // Se só uma das séries tiver dados, tenta compensar mantendo o tamanho das labels.
        $entradaSeries = $entradas->pluck('quantidade')->map(fn ($v) => (float) $v)->all();
        $saidaSeries = $saidas->pluck('quantidade')->map(fn ($v) => (float) $v)->all();

        // Ajuste simples de tamanho para não quebrar o chart.
